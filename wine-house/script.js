...
window.addEventListener('wheel', (e) => {
  ...
  if (bool) {
    ...
    document.querySelector(`.section-${deltaY ? counter1 : counter2}-wrapper`)
      .style.transform = `scale(${deltaY ? '1.5' : '1'})`

    document.querySelector(`.section-${deltaY ? counter1 + 1 : counter2 + 1}-wrapper`)
...
document.querySelector('.left-btn')
  .addEventListener('click', () => {
    ...
    if (bool) {
      ...
      if (document.querySelector(`.section-${counter1}-wrapper`)) {
...
document.querySelector('.right-btn')
  .addEventListener('click', () => {
    ...
    if (bool) {
      document.querySelector(`.section-${counter2}-wrapper`).style.transform = 'scale(1)'
      if (document.querySelector(`.section-${counter2 + 1}-wrapper`)) {
        document.querySelector(`.section-${counter2 + 1}-wrapper`).style.transform = 'scale(1.5)'
      }
    }
  })

document.querySelector('.grapes-img')
